...
def read_parquet_optimized(items: Iterator[FileItemDict], data_type: Optional[str] = None,
    ...
    """
    Read Parquet files with selective processing based on data type, year and months.
    ...
    for file_obj in items:
        #logger.info(f"FileItemDict content: {json.dumps(file_obj, indent=2)}")
        ...
        if match:
            ...
            try:
                with file_obj.open() as f:
                    # Use pandas to read parquet in chunks to save memory
